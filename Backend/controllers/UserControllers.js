...
export const updateUser = async (req, res) => {
  try {
    ...
    const updateFields = { name, email, phone, address, dateOfBirth, specialization };

    // admin may modify role or activation status
...
export const createUser = async (req, res) => {
  try {
    const { name, email, password, role, specialization, contactNumber } = req.body;
    ...
    if (!allowedRoles.includes(normalizedRole)) {
      return res.status(400).json({
        success: false,
        message: 'Invalid role specified. Allowed: admin, doctor, receptionist, patient'
      });
    }
